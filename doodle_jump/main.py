...
from file_manager import get_image, get_font, get_sound
from player import Player, Monster
from boost import StaticBoost, RedBoost, MovementBoost, SpringBoost
from screen import Start, Pause
from random import randint

from static import load_results, save_results
...
class App:
    def __init__(self):
        ...
        self.game_over_bg = pygame.image.load(
            get_image("game_over.png"),
        )
        self.start_screen_bg = pygame.image.load(
            get_image("start_screen_bg.png"),
        )
        self.pause_screen_bg = pygame.image.load(
            get_image("pause.png"),
        )
        ...
        self.font = get_font("al-seana.ttf")
    ...
    def generate_level(self):
        ...
        while len(self.boosts) < self.n_boosts:
            coord = [
                ...
                self.boosts.sprites()[-1].rect.y - random.randint(50, 80),
            ]
            if self.cntr:
                if self.boosts.sprites()[-1].rect.x not in range(80, 194):
                    coord = [
                        random.randint(80, 194),
                        self.boosts.sprites()[-1].rect.y - random.randint(
                            25, 50,
                        ),
                    ]
                elif self.boosts.sprites()[-1].rect.x not in range(406, 520):
                    coord = [
                        random.randint(406, 520),
                        self.boosts.sprites()[-1].rect.y - random.randint(
                            25, 50,
                        ),
                    ]
    ...
    def start(self):
        ...
        while True:
            ...
            if not self.flag_monster and pygame.sprite.collide_rect(
                    self.monsters.sprites()[0], self.pl):
                return self.game_over()
            pygame.display.flip()
